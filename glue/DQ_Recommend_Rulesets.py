...
if 'CodeBucket' not in os.environ.keys():
    os.environ['CodeBucket'] = 'wc2h-dtl-prd-code'  
...
s3_client.upload_file( filename, 'wc2h-dtl-prd-datawork', f'rmyers07/{filename}')
'''
# Instantiate DQ 
...
for batch_item in batch['BatchObjects']:
    ...
    try:
        suggestion_run_id = batch_item['Step-DQ_Suggestions']['SuggestionRunId']
        rec = dq.get_data_quality_rule_recommendation_run( RunId = suggestion_run_id )
        rules_list = []
